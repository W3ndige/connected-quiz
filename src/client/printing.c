...
bool getPrintQuestion(SDL_Surface *screen, SDL_Surface *background, TTF_Font *font, SDL_Surface *text_surface, int socket_fd, SDL_Rect text_location, SDL_Rect answer_location, SDL_Color foreground_color) {
  ...
  printf("%d\n", text_width); // width of our text
  if (text_width > 425) {
    char first_line[51];
    memset(first_line, 0, 51);
    strncpy(first_line, buffer, 50);
    first_line[51] = '\0';
    printf("%s\n", first_line);
...
bool printGameMode(SDL_Surface *screen, SDL_Surface *background, TTF_Font *font, TTF_Font *header_font, SDL_Surface *text_surface, SDL_Rect mode_location, SDL_Color foreground_color) {
  ...
  SDL_BlitSurface(background, NULL, screen, &background_location);
  text_surface = TTF_RenderText_Blended(header_font, "Welcome to Connected Quiz!", foreground_color);
  SDL_BlitSurface(text_surface, NULL, screen, &welcome_message);
  for (int i = 0; i < NUM_OF_MODES; i++) {
    text_surface = TTF_RenderText_Blended(font, game_options[i], foreground_color);
